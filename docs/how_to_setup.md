...
AWS CLI が動作している場合は aws configure で認証情報やデフォルトリージョンを設定しておきます
(Bedrock が対応しているリージョン us-west-2 も併せて設定・利用するため)
...
8-1. AWS CLI / CDK の設定
aws configure にて AWS 認証情報とデフォルトリージョンを設定

Bedrock が us-west-2 リージョンなので、利用するプロファイルでリージョン設定も適宜行う
